...
import {
    ...
    invalidateCache as invalidateRajeshFlatCache
} from "../services/rajeshFlatService";
...
const DashboardPage = ({ user, onLogout, onLogin }) => {
    ...
    const navigateToEditForm = useCallback((record) => {
        ...
        } else if (record?.selectedForm === 'rajeshflat') {
            // Rajesh Flat form route
    ...
    const fetchFiles = async (isInitial = false, showLoadingIndicator = true, page = 1) => {
        try {
            ...
            // Fetch only ITEMS_PER_PAGE_API items per page (10 items) for faster initial load
            const valuationsResponse = await getAllValuations({ username, userRole: role, clientId, limit: ITEMS_PER_PAGE_API, page }).catch(() => ({ data: [] }));

            // Fetch other endpoints in parallel with same page size
            const [bofResponse, ubiApfResponse, rajeshHouseResponse, rajeshBankResponse, rajeshFlatResponse, rajeshRowHouseResponse] = await Promise.all([
                getAllBofMaharashtra({ username, userRole: role, clientId, limit: ITEMS_PER_PAGE_API, page }).catch(() => ({ data: [] })),
                getAllUbiApfForms({ username, userRole: role, clientId, limit: ITEMS_PER_PAGE_API, page }).catch(() => ({ data: [] })),
                getAllRajeshHouse({ username, userRole: role, clientId, limit: ITEMS_PER_PAGE_API, page }).catch(() => ({ data: [] })),
                getAllRajeshBank({ username, userRole: role, clientId, limit: ITEMS_PER_PAGE_API, page }).catch(() => ({ data: [] })),
                getAllRajeshFlat({ username, userRole: role, clientId, limit: ITEMS_PER_PAGE_API, page }).catch(() => ({ data: [] })),
            ...
            const rajeshFlatData = (Array.isArray(rajeshFlatResponse?.data) ? rajeshFlatResponse.data : [])
                .map(item => ({ ...item, formType: 'rajeshflat', selectedForm: item.selectedForm || 'rajeshflat' }));
            ...
            const response = {
                ...valuationsResponse,
                data: [...valuationsData, ...bofData, ...ubiApfData, ...rajeshHouseData, ...rajeshBankData, ...rajeshFlatData, ...rajeshRowHouseData]
            };
            ...
            if (isInitial) {
                ...
                setFiles(prevFiles => {
                    const combined = [...prevFiles, ...filesList];
                    ...
                    combined.forEach(item => {
                        ...
                        const existing = uniqueMap.get(item.uniqueId);
                        if (!existing) {
                            uniqueMap.set(item.uniqueId, item);
                            merged.push(item);
                        } else {
                            const existingTime = new Date(existing.lastUpdatedAt || existing.updatedAt || existing.createdAt).getTime();
                            const currentTime = new Date(item.lastUpdatedAt || item.updatedAt || item.createdAt).getTime();
                            ...
                            if (currentTime > existingTime) {
                                const idx = merged.findIndex(d => d.uniqueId === item.uniqueId);
                                merged[idx] = item;
                                uniqueMap.set(item.uniqueId, item);
                            }
                        }
    ...
    const handleDownloadPDF = async (record) => {
        try {
            ...
            if (record?.selectedForm === 'bomFlat') {
                ...
                // Use Rajesh Flat PDF service for rajeshflat
                const { generateFlatRajesh } = await import("../services/rajeshFlatPdf.js");
                await generateFlatRajesh(record);
            } else if (record?.selectedForm === 'rajeshrowhouse') {
            ...
            // Special handling for rajeshbank - auto approve after download
            if (record?.selectedForm === "rajeshbank") {
                try {
                    const { managerSubmitRajeshBank } = await import("../services/rajeshBankService.js");
                    const user = localStorage.getItem("user");
                    const userData = user ? JSON.parse(user) : {};

                    await managerSubmitRajeshBank(
                        record.uniqueId,
                        "approved",
                        "Auto-approved after PDF download",
                        userData.username || "system",
                        userData.role || "admin"
                    );

                    showSuccess("Record auto-approved and removed!");
                    // Refresh files to show updated status
                    await fetchFiles(false, false);
                } catch (approvalError) {
                    console.error("Auto-approval error:", approvalError);
                    showError(approvalError.message || "Failed to auto-approve record");
                }
            }
        } catch (error) {
    ...
    const handleReworkSubmit = async (reworkComments) => {
        try {
            ...
            if (reworkingRecord?.formType === 'bomFlat') {
                ...
                invalidateCache("rajesh-bank");
            } else if (reworkingRecord?.formType === 'rajeshflat') {
                ("[handleReworkSubmit] Calling requestReworkRajeshFlat");
                await requestReworkRajeshFlat(reworkingRecordId, reworkComments, username, role);
                invalidateCache("rajesh-flat");
                                                                                ...
                                                                                record.selectedForm === 'ubiApf' ? 'bg-gradient-to-r from-amber-500 to-amber-600' :
                                                                                    record.selectedForm === 'rajeshhouse' ? 'bg-gradient-to-r from-green-500 to-green-600' :
                                                                                        record.selectedForm === 'rajeshbank' ? 'bg-gradient-to-r from-red-500 to-red-600' :
                                                                                            record.selectedForm === 'rajeshflat' ? 'bg-gradient-to-r from-cyan-500 to-cyan-600' :
                                                                                    ...
                                                                                    record.selectedForm === 'ubiApf' ? 'UBI APF' :
                                                                                        record.selectedForm === 'rajeshhouse' ? 'Rajesh House' :
                                                                                            record.selectedForm === 'rajeshbank' ? 'Rajesh Bank' :
                                                                                                record.selectedForm === 'rajeshflat' ? 'Rajesh Flat' :
