...
const BillForm = ({ user }) => {
    ...
    const handleRemoveSelectedRow = async (rowId) => {
        // Create updated rows list
        const updatedRows = selectedRows.filter(row => row._id !== rowId);

        // Remove the data for this row
        setSelectedRowsData(prev => {
            const newData = { ...prev };
        ...
        // If in editing mode, update the database
        if (id) {
            try {
                setLoading(true);

                // Prepare the full bill update with new selectedRecords
                const updateData = {
                    ...billData,
                    items,
                    selectedRecords: updatedRows.map(row => ({
                        ...row,
                ...
                ("[BillForm] Removing record, updating bill with selectedRecords:", updateData.selectedRecords);

                const response = await updateBill(id, updateData);
                if (response && response.success) {
                    setSelectedRows(updatedRows);
                    showSuccess("Record removed successfully");
                } else {
                    showError(response?.message || "Failed to remove record");
                    // Revert if API call failed
                    setSelectedRows(prev => [...prev, selectedRows.find(row => row._id === rowId)]);
                }
            } catch (err) {
                console.error("[BillForm] Error removing record:", err);
                showError(err.response?.data?.message || "Error removing record");
                // Revert if API call failed
                setSelectedRows(prev => [...prev, selectedRows.find(row => row._id === rowId)]);
            } finally {
                setLoading(false);
    ...
    const handleConfirmClearAll = async () => {
        ...
        if (id) {
            try {
                ...
                // Prepare the full bill update with empty selectedRecords
                const updateData = {
                    ...billData,
                    items,
                    selectedRecords: []
                ...
                ("[BillForm] Clearing all records, updating bill with empty selectedRecords");
                ...
                if (response && response.success) {
                    ...
                    showError(response?.message || "Failed to clear records");
                }
            } catch (err) {
                console.error("[BillForm] Error clearing records:", err);
                showError(err.response?.data?.message || "Error clearing records");
            } finally {
                setLoading(false);
            }
        } else {
            // In create mode, just clear frontend state
            setSelectedRows([]);
            setSelectedRowsData({});
        }
    };
    ...
    return (
        <div className="min-h-screen bg-slate-50 p-4">
                                ...
                                {selectedRows.length > 0 && (
                                    <div className="border-t pt-4">
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <button
                                                type="button"
                                                onClick={handleClearAllSelectedRows}
                                                className="text-sm px-3 py-1 bg-red-100 hover:bg-red-200 text-red-700 hover:text-red-900 rounded transition-colors"
                                            >
                                                Clear All
                                            </button>
                                        </div>
                                            ...
                                            <table className="w-full text-sm">
                                                ...
                                                <tbody>
                                                    {selectedRows.map((row, index) => (
                                                        <tr key={index} className="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                                                            ...
                                                            <td className="px-4 py-2 text-center">
                                                                <button
                                                                    type="button"
                                                                    onClick={() => handleRemoveSelectedRow(row._id)}
                                                    ...
                                                    <tr className="bg-slate-100 border-t-2 border-slate-300">
                                                        <td colSpan="7" className="px-4 py-3 text-right font-bold text-slate-900">Total Fee:</td>
            ...
            <Dialog open={showClearConfirmation} onOpenChange={setShowClearConfirmation}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Clear All Selected Records</DialogTitle>
                        <DialogDescription>
                            Are you sure you want to clear all {selectedRows.length} selected records? This action cannot be undone.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <Button
                            ...
                            variant="outline"
                            className="border border-slate-300 text-slate-900 hover:border-slate-400 hover:bg-slate-50"
                        >
                            Cancel
                        </Button>
                        <Button
