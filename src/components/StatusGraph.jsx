...
const StatusGraph = memo(({ files, isCompact = false }) => {
    ...
    const statusData = useMemo(() => {
        ...
        return [
            ...
            { name: "Completed", value: statusCounts.completed, fill: "#06b6d4", lightFill: "#cffafe", darkFill: "#164e63" },
        ];
    }, [files]);
                ...
                <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0 p-3 space-y-3 scrollbar-thin scrollbar-thumb-blue-400 scrollbar-track-gray-100">
                            ...
                            {/* 6 Status Charts - Colorful Grid */}
                             <div className="grid grid-cols-6 gap-4">
                                 {statusData.map((status, idx) => {
                                     ...
                                     const percentage = totalCount > 0 ? ((status.value / totalCount) * 100).toFixed(1) : 0;

                                     return (
                                         <div key={idx} className={`relative overflow-hidden flex flex-col items-center justify-between rounded-2xl border-2 ${borderColors[idx]} bg-gradient-to-br ${bgGradients[idx]} hover:shadow-2xl ${shadowColors[idx]} transition-all duration-300 group`} style={{ minHeight: '160px', padding: '8px' }}>
                                             {/* Animated background accent */}
                                             <div className={`absolute -top-6 -right-6 w-24 h-24 bg-gradient-to-br ${headerGradients[idx]} opacity-10 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500`}></div>

                                             {/* Header with gradient background */}
                                             <div className={`absolute top-0 left-0 right-0 h-8 bg-gradient-to-r ${headerGradients[idx]} flex items-center justify-center rounded-t-lg`}>
                                                 <p className="text-xs font-black text-white uppercase tracking-widest drop-shadow-lg">{status.name}</p>
                                             </div>

                                             {/* Chart content */}
                                             <div className="relative z-10 flex flex-col items-center justify-center w-full" style={{ marginTop: '30px' }}>
                                                 {/* Donut Chart */}
                                                 <div style={{ height: '60px', width: '100%' }}>
                                                     <ResponsiveContainer width="100%" height="100%">
                                                         <PieChart>
                                                             <Pie
                                                                 data={[
                                                                     { name: status.name, value: status.value || 1 },
                                                                     { name: "empty", value: totalCount - status.value || 0 }
                                                                 ]}
                                                                 cx="50%"
                                                                 cy="50%"
                                                                 innerRadius={14}
                                                 ...
                                                 <div className="text-center" style={{ marginTop: '-8px' }}>
                                                     <p className="text-sm font-black text-gray-900 leading-tight">{percentage}%</p>
                                                     <p className="text-xs font-semibold text-gray-600 leading-tight">{status.value}</p>
                                                 </div>
                                             </div>
                                         </div>
                                     );
                    ...
                    {activeTab === "cities" && cityData.length > 0 && (
                        <div className="space-y-2.5">
                            ...
                            {/* Premium Chart Container */}
                            <div className="relative overflow-visible rounded-xl border border-slate-200/50 shadow-sm hover:shadow-md transition-all duration-300 group bg-white">
                                <div className="absolute inset-0 bg-gradient-to-br from-amber-50/30 via-transparent to-slate-50/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                                <div className="absolute -top-16 -right-16 w-32 h-32 bg-amber-400/5 rounded-full blur-3xl group-hover:scale-110 transition-transform duration-500"></div>
                                <div className="relative z-10 p-4">
                                    <div className="flex items-center gap-2 mb-3 pb-2 border-b border-slate-200">
                                        <div className="w-1 h-5 bg-gradient-to-b from-amber-600 to-amber-400 rounded-full shadow-md"></div>
                                        <p className="text-xs font-bold text-slate-700 uppercase tracking-widest">City-wise Submission Volume</p>
                                    </div>
                                    <ResponsiveContainer width="100%" height={180}>
                                        <PieChart>
                                            <Pie
                                                data={cityData.map((item) => ({ ...item, value: item.count }))}
                                                cx="50%"
                                                cy="50%"
                                                labelLine={true}
                                                label={({ name, value, percent }) =>
                                            ...
                                            >
                                                {cityData.map((entry, index) => {
                                                    ...
                                                    return <Cell key={`cell-${index}`} fill={colors[index % colors.length]} opacity={0.9} />;
                                                })}
                                            </Pie>
                                            <Tooltip content={<CustomTooltip />} />
                                        </PieChart>
                            ...
                            <div className="relative overflow-visible rounded-xl border border-slate-200/50 shadow-sm hover:shadow-md transition-all duration-300 group bg-white">
                                ...
                                <div className="relative z-10 p-4">
                                    <div className="flex items-center gap-2 mb-3 pb-2 border-b border-slate-200">
                                        ...
                                        <p className="text-xs font-bold text-slate-700 uppercase tracking-widest">Monthly Submission Volume</p>
                                    </div>
                                    <ResponsiveContainer width="100%" height={180}>
                                        <PieChart>
                                            <Pie
                                                ...
                                                cy="50%"
                                                labelLine={true}
                                                label={({ name, submissions, percent }) =>
                                                    `${name}: ${submissions} (${(percent * 100).toFixed(0)}%)`
                                                }
                                                outerRadius={50}
                                                innerRadius={22}
                                                dataKey="submissions"
                                            ...
                                            >
                                                {analyticsData.monthlyData.map((entry, index) => {
                                                    const colors = ["#4f46e5", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6", "#ec4899", "#06b6d4", "#14b8a6", "#f97316", "#6366f1", "#d946ef", "#f43f5e"];
                                                    return <Cell key={`cell-${index}`} fill={colors[index % colors.length]} opacity={0.9} />;
                                                })}
                                            </Pie>
                                            <Tooltip content={<CustomTooltip />} />
                                ...
                                <div className="lg:col-span-2 relative overflow-visible rounded-2xl border border-neutral-300 shadow-md hover:shadow-lg transition-all duration-500 group bg-neutral-0">
                                    ...
                                    <div className="relative z-10 p-3">
                                        <div className="flex items-center gap-2 mb-2 pb-2 border-b border-gray-200">
                                            ...
                                            <p className="text-xs font-bold text-gray-800 uppercase tracking-widest">Engineer Performance Distribution</p>
                                        </div>
                                        <ResponsiveContainer width="100%" height={220}>
                                            <PieChart>
                                                <Pie
                                                    data={analyticsData.engineerStats}
                                                    cx="50%"
                                                    cy="50%"
                                                    labelLine={true}
                                                    label={({ name, total, percent }) =>
                                                        `${name}: ${total} (${(percent * 100).toFixed(0)}%)`
                                                    }
                                                    outerRadius={48}
                                                    innerRadius={20}
                                                    dataKey="total"
                                                    ...
                                                    isAnimationActive={true}
                                                >
                                                    {analyticsData.engineerStats.map((entry, index) => {
                                                        const colors = ["#ec4899", "#f43f5e", "#f97316", "#d946ef", "#a855f7", "#7c3aed", "#6366f1", "#3b82f6", "#06b6d4", "#14b8a6"];
                                                        return <Cell key={`cell-${index}`} fill={colors[index % colors.length]} opacity={0.9} />;
                                                    })}
                                                </Pie>
                                                <Tooltip content={<CustomTooltip />} />
