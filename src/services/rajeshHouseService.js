...
/**
  ...
  * @param {String} username - Current user
  * @param {String} clientId - Client identifier
  * @param {String} selectedFormName - Selected form type (optional - filters by form type)
  * @returns {Promise} Last form data with pdfDetails and spreadsheetData
  */
export const getLastSubmittedRajeshHouse = async (username, clientId, selectedFormName = null) => {
  try {
    if (!username || !clientId) {
      throw new Error('Missing required parameters: username and clientId');
    }

    const params = { username, clientId };
    if (selectedFormName) {
      params.selectedFormName = selectedFormName;
    }

    console.log("[getLastSubmittedRajeshHouse] Calling API endpoint with params:", params);
    
    const response = await api.get(`${API_BASE_URL}/rajesh-house/last-form/prefill`, { params });
    console.log("[getLastSubmittedRajeshHouse] API Response:", {
      success: response.data.success,
      message: response.data.message,
      hasData: !!response.data.data,
      dataKeys: response.data.data ? Object.keys(response.data.data).slice(0, 10) : [],
      hasSpreadsheetData: response.data.data?.spreadsheetData ? true : false,
      selectedFormName: response.data.data?.selectedFormName
    });
    ...
    console.error("[getLastSubmittedRajeshHouse] Error:", {
